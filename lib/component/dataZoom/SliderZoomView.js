...
var SliderZoomView = DataZoomView.extend({
  ...
  _renderDataShadow: function () {
    ...
    if (axisOptions !== 'category') {
      ...
      var thisShadowExtent = [Math.max((dataExtent[0] - dataMin) / (dataMax - dataMin) * size[0], 0), Math.min((dataExtent[1] - dataMin) / (dataMax - dataMin) * size[0], size[0])];
    } else {
    ...
    var steps = thisShadowExtent[0]; // Optimize for large data shadow
    ...
    data.each([info.thisDim, otherDim], function (thisValue, value, index) {
      ...
      var thisCoord = axisOptions !== 'category' ? (thisValue - dataMin) / (dataMax - dataMin) * size[0] : steps; // FIXME
